...
function evaluateRules(bundle: EvidenceBundle, findings: Finding[], policy: VerificationPolicy): void {
  const presentKinds = new Set(bundle.claims.map((claim) => claim.kind));
  policy.requiredKinds.forEach((kind) => {
  ...
  for (const claim of bundle.claims) {
    ...
    const mode = replayMode(claim);
    if (claim.effect?.external && mode === "non-replayable") {
      findings.push({
        ruleId: "replay.external-effect",
        severity: policy.externalEffectSeverity,
        title: policy.externalEffectSeverity === "pass" ? "External effect accepted by policy" : "External effect cannot be replayed safely",
        detail: "The effect is recorded for audit, but replay requires an explicit sandbox adapter.",
        claimId: claim.id,
      });
    }
    if (mode === "unknown" && policy.unknownReplaySeverity !== "pass") {
      findings.push({
        ruleId: "replay.unknown-mode",
        severity: policy.unknownReplaySeverity,
        title: "Replay mode is not declared",
        detail: "The claim has evidence, but its replay semantics are not declared by the bundle.",
        claimId: claim.id,
      });
    }
  }
  ...
  if (bundle.envelope?.type !== "unsigned" && bundle.envelope?.verified !== true) {
    findings.push({
      ...
      severity: policy.requireVerifiedEnvelope ? "block" : "review",
      title: policy.requireVerifiedEnvelope ? "Envelope verification is required" : "Envelope is not cryptographically verified",
...
export async function verifyBundle(input: unknown, options: { policy?: VerificationPolicy } = {}): Promise<VerificationResult> {
  ...
  evaluateRules(bundle, findings, policy);
