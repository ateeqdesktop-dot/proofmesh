...
export function declaredSignatureStatus(
  bundle: EvidenceBundle
): SignatureStatus {
  if (!bundle.envelope || bundle.envelope.type === "unsigned")
    return "unsigned";
  if (bundle.envelope.signature)
    return bundle.envelope.verified ? "declared" : "declared";
  return "declared";
}
