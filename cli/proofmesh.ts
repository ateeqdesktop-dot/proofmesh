...
import { verifyWithCosign } from "./cosign";
...
function usage(): never {
  console.error(`ProofMesh ${VERSION}\n\nUsage:\n  pnpm proofmesh verify <bundle.json> [--policy policy.json] [--dsse-envelope envelope.json --public-key key.pem] [--cosign-key key.pub --cosign-signature bundle.sig] [--format json|sarif] [--output report.json]\n\nExit codes:\n  0  pass\n  1  review findings\n  2  blocked or invalid bundle\n`);
  process.exit(2);
}
...
async function main() {
  ...
  const cosignKeyIndex = args.indexOf("--cosign-key");
  const cosignKeyPath = cosignKeyIndex >= 0 ? args[cosignKeyIndex + 1] : undefined;
  const cosignSignatureIndex = args.indexOf("--cosign-signature");
  const cosignSignaturePath = cosignSignatureIndex >= 0 ? args[cosignSignatureIndex + 1] : undefined;
  if (Boolean(envelopePath) !== Boolean(publicKeyPath) || Boolean(cosignKeyPath) !== Boolean(cosignSignaturePath) || (envelopePath && cosignKeyPath)) {
  ...
  if (cosignKeyPath && cosignSignaturePath) {
    const result = await verifyWithCosign(source, resolve(cosignKeyPath), resolve(cosignSignaturePath));
    finalReport.signature = result;
    if (!result.verified) {
      finalReport.findings.push({ ruleId: "signature.cosign-invalid", severity: "block", title: "Cosign signature could not be verified", detail: result.reason ?? "Cosign rejected the bundle signature.", path: cosignSignaturePath });
      finalReport.verdict = "block";
    }
  } else if (envelopePath && publicKeyPath) {
    try {
