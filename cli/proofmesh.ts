...
async function main() {
  ...
  const publicKeyPath =
    publicKeyIndex >= 0 ? args[publicKeyIndex + 1] : undefined;
  if (Boolean(envelopePath) !== Boolean(publicKeyPath)) {
    console.error(
      "ProofMesh: --dsse-envelope and --public-key must be provided together."
    );
    process.exit(2);
  }
  ...
  if (envelopePath && publicKeyPath) {
    try {
      const envelopeInput = JSON.parse(
      ...
      const result = envelope
        ...
        : { verified: false, reason: "DSSE envelope is invalid." };
      ...
      if (!result.verified) {
        finalReport.findings.push({
          ...
          detail:
            result.reason ??
            "The supplied signature did not verify against the canonical bundle.",
          path: envelopePath,
        });
      ...
      console.error(
        `ProofMesh: unable to verify DSSE envelope: ${error instanceof Error ? error.message : String(error)}`
      );
      process.exit(2);
    }
  }
  const payload =
    format === "sarif" ? toSarif(finalReport, args[1]) : finalReport;
