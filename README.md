...
## Optional Cosign / Sigstore bridge

For CI environments that already install Cosign, the reusable action can delegate blob verification without invoking a shell or discovering trust roots. Pass the bundle's public key and signature explicitly:

```yaml
- uses: ateeqdesktop-dot/proofmesh/.github/actions/verify@main
  with:
    bundle: examples/passing-bundle.json
    cosign-key: security/proofmesh.pub
    cosign-signature: evidence/passing-bundle.sig
    fail-on-review: 'false'
```

ProofMesh does not install Cosign, contact Fulcio or Rekor, resolve OIDC identity, or accept a `signer` label as proof. The caller owns Cosign installation, key distribution, trust-root configuration, and the decision to pin the action to a release tag. Verification failures are reported as `signature.cosign-invalid` and produce a blocked verdict.
